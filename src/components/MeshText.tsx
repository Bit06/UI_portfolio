...
function renderTextToCanvas(
    text: string, color: string, fontFamily: string, fontWeight: string | number,
    fontStyle: string, fontSize: number, width: number, height: number
): HTMLCanvasElement {
...
const COMPONENT_DEFAULTS = {
    text: "MESH",
    color: "#ffffff",
    font: { fontFamily: "Inter", variant: "Bold", fontSize: 180, lineHeight: "1em" } as any,
    colorSplit: true,
...
export default function MeshText(props: any) {
    ...
    useEffect(() => {
        ...
        let rafId = 0
        const tick = () => {
            // Auto-animate cursor in a figure-8 or circular pattern
            const t = performance.now() * 0.0015; // Speed of auto-animation
            const autoX = Math.sin(t) * 0.8;
            const autoY = Math.cos(t * 1.3) * 0.6; // slightly off-phase for complex movement
            ...
            for (let i = 0; i < vertCount; i++) {
                const i2 = i * 2; const px = positions[i2]; const py = positions[i2 + 1];
                const dx = disp[i2]; const dy = disp[i2 + 1];
                const cx = cursor.x - (px + dx); const cy = cursor.y - (py + dy);
                const proximity = Math.max(0, 1 / (1 + Math.hypot(cx, cy) / 0.1) - 0.1)

                let vx = vel[i2]; let vy = vel[i2 + 1];
                const fpull = forceRef.current
                vx += cursor.vx * fpull * proximity * 2.0; // Boosted force for auto-animation
                vy += cursor.vy * fpull * proximity * 2.0;
            ...
            if (cols.length === 1) { cA = cols[0]; cB = cols[0]; }
                ...
                const idx = Math.floor(performance.now() / 400) % cols.length
        ...
        return () => {
            ...
            wrapper.removeEventListener("pointermove", onMove); wrapper.removeEventListener("pointerleave", onLeave);
            gl.deleteBuffer(posBuf); gl.deleteBuffer(uvBuf); gl.deleteBuffer(dispBuf); gl.deleteBuffer(idxBuf);
            gl.deleteTexture(tex); gl.deleteVertexArray(vao); gl.deleteProgram(program); gl.deleteShader(vs); gl.deleteShader(fs);
        }
    }, [text, color, fontFamily, fontWeight, fontStyle, fontSize])

    return (
        <div ref={wrapperRef} style={{ position: "relative", width: "100%", height: "100%", overflow: "visible", userSelect: "none" }}>
            <canvas ref={canvasRef} style={{ display: "block", width: "100%", height: "100%", overflow: "visible" }} />
        </div>
    )
}
